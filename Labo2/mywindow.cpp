...
void MyWindow::on_pushButtonLogin_clicked()
{
  ...
  int nouvelUtilisateur, pos;
  strcpy(nom,getNom());
  strcpy(motDePasse,getMotDePasse());
  nouvelUtilisateur = isNouveauChecked(); //1 si il est nouveau 0 si non

  // TO DO
  if(nouvelUtilisateur == 1) // si on doit créer un user
  {
    if(estPresent(nom) != 0)
    {
